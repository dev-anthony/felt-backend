...
const PERSON_SUBJECT_BLOCK = `SUBJECT CONSTRUCTION (choose WHO fits the song, and make them MEMORABLE):
- YOUTH CULTURE & DEMOGRAPHIC ANCHORS: Explicitly dictate modern, fashion-forward young figures. Gen-Z or Millennial artists, contemporary urban fashion, athletic or slender builds. NEVER generate elderly subjects, random old men, or "classic wisdom" figures unless strictly demanded by the lyrics.
- STYLING & EDGE: Detail edge-cut hairstyles (dreads, locs, braids, Bantu knots, slicked hair, clean fades). Include modern distinctive markers (band-aids on noses, leather corsets, futuristic sunglasses, streetwear jackets, gold/silver jewelry, face/hand tattoos, signature hats).
- ANATOMY: state build and one or two bone-structure facts so the figure has real mass.
- SKIN: name a base tone from a real spectrum (porcelain, warm ivory, golden olive, honey-bronze, rich caramel, deep espresso, obsidian) and one micro-texture (freckles, visible pores, sun-weathered lines, healed scars).
- WARDROBE WITH WEIGHT: name the garment AND how the fabric behaves under gravity.
- BANNED words for people: "elderly", "old man", "wrinkled wisdom", "beautiful", "stunning", "gorgeous", "attractive", "perfect", "athletic", "sculptural", "high-fashion figure", "enigmatic", "mysterious figure", "a person", "someone", "cool outfit", "stylish".
- The face is LIT and clearly visible, unless intentionally concealed by conceptual action (e.g., holding a Polaroid over the face). Never describe it as shadowed or turned away UNLESS the technique is SILHOUETTE_ATMOSPHERE or MONUMENTAL_SCALE_ISOLATION.`

const ABSENT_SUBJECT_BLOCK = `OBJECT / ABSTRACT CONSTRUCTION (this cover has NO human figure — that is deliberate):
- The Visual DNA has determined this song is carried by a thing, a place or a material rather than a person. Nobody is in this picture.
- Choose ONE concrete subject and commit to it entirely: a single object loaded with the song's meaning, one material caught in a specific state, or one structure.
- Give it PHYSICALITY: what it is made of, how it has been used, what time has done to it. Chips, wear, fingerprints, dust, condensation, heat damage, repair. An object with no history reads as stock.
- Give it SCALE and PLACEMENT: whether we are inches from it or across a room, and what it sits on or in.
- Give it a MOMENT: even without a person something is happening.
- BANNED words: "beautiful", "stunning", "ethereal", "abstract shapes", "an object", "a surface", "some kind of", "mysterious".`

const MEDIUM_BRIEF = {
  photo: {
    opener: 'You write ONE photographic moment. Lock into 100% photorealistic analog texture: shot on 35mm film or medium format analog camera, coarse film grain, raw skin texture, direct camera flash shadows, chromatic aberration, subtle motion blur, low-light ISO noise, volumetric haze.',
    execution: 'photography',
    forbid: '3D render, digital painting, smooth skin, plastic texture, CGI, over-sharpened, airbrushed, cartoon, fantasy illustration',
  },
  cgi: {
    opener: 'You write ONE rendered moment. This cover will be BUILT as a 3D render, not photographed — so think in surfaces, materials and constructed space, not in what a camera happened to catch.',
    execution: 'rendering',
...
function aestheticSystemPrompt({ mediumFamily = 'photo', subjectMode = 'person' } = {}) {
  const M = MEDIUM_BRIEF[mediumFamily] || MEDIUM_BRIEF.photo
  const subjectBlock = subjectMode === 'absent' ? ABSENT_SUBJECT_BLOCK : PERSON_SUBJECT_BLOCK
  return `You are an avant-garde creative director for a cutting-edge music artist. ${M.opener} You do not write stock clichés, repetitive portraits, or predictable layouts.

Everything you write must serve one goal: complete artistic freedom and visual originality matched to this exact song. Avoid defaulting to centered single-person portraits unless the emotion strictly demands it. Embrace asymmetry, wide environments, surreal conceptual objects (e.g., a subject reading a burning newspaper), graphic illustration perspectives, and bold CGI spatial concepts.

An EMOTIONAL REGISTER block is supplied with every brief. Let it govern an expansive, unconstrained visual translation—exploring scale, mood, texture, and unexpected focal points.
...
CREATIVE VARIETY & FREEDOM MANDATE:
- Actively VARY compositions: utilize extreme wide angles, dramatic macro close-ups, top-down perspectives, asymmetrical negative space, and environmental storytelling.
- Do NOT default to a standard centered human figure. Let the music decide whether the frame features a sweeping landscape, an expressive object, abstract 3D forms, or an unconventional portrait layout.
...
Respond with exactly two lines, nothing else:
TECHNIQUE: <one of the 10 technique names above, exact match>
SCENE: <2-3 sentence striking cinematic or artistic moment grounded in this song. Break free from standard portrait tropes; prioritize unique framing, environmental scale, and distinct artistic medium aesthetics. No camera, lighting, colour or grain words. No preamble, no quotes, no lyric excerpts.>`
}
...
function deriveSceneMode(features) {
  try {
    ...
    // Balanced medium selection: Photography no longer requires a > 50% handicap. 
    // All mediums (photo, illustration, cgi) have an equal opportunity based on top DNA scoring.
    const top = Object.entries(mediums).sort((a, b) => b[1] - a[1])[0]
    const decisive = top ? top[0] : 'photo'
...
function buildFluxPrompt(technique, scene) {
  const suffix = TECHNIQUE_SUFFIXES[technique] || TECHNIQUE_SUFFIXES[DEFAULT_TECHNIQUE]
  return `${scene}. ${suffix} Definitively moody, intentional, and authentic — zero digital smoothing, zero CGI artifacts, zero plastic AI skin. Absolutely no 3D render, digital painting, smooth skin, plastic texture, CGI, over-sharpened, airbrushed, cartoon, fantasy illustration.`
}
...
async function buildFinalPrompt(technique, scene, features, { useCompiler = false, userFeeling, mood, noPeople = false, mediumFamily } = {}) {
  try {
    ...
    // Inject strong negative prompts into the final assembled DNA to kill the "AI Look"
    const antiAILook = " Absolutely no 3D render, digital painting, smooth skin, plastic texture, CGI, over-sharpened, airbrushed, cartoon, fantasy illustration."
    return { prompt: built.prompt + antiAILook, technique: built.technique, dna: built.dna }
  } catch (err) {
    console.warn(`[ENGINE] Visual DNA build failed, using legacy prompt: ${err?.message || err}`)
    return { prompt: buildFluxPrompt(technique, scene), technique, dna: null }
  }
