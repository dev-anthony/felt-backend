...
// Recovers { technique, scene } from a stored sentence_prompt value. `structured`
// tells the caller whether this came from real JSON (a finished /expand or
// /transcribe brief) or was a raw, unexpanded string that just happens to be
// non-empty — so a caller can choose NOT to treat raw text as a literal scene.
function deserializeBrief(stored) {
  ...
  try {
    ...
    // Not JSON — raw, unexpanded input left in this column (e.g. a beat
    // generated before the /expand persistence fix, or any other path that
    // never ran it through AESTHETIC_SYSTEM_PROMPT).
  }
...
${artist.contextLine ? `Artist Branding Space Context: ${artist.contextLine}` : ''}`;
    ...
    // Persist technique + scene together, same as /transcribe — otherwise
    // sentence_prompt stays the raw one-liner the artist typed, and /generate's
    // deserializeBrief later fails to parse it as JSON, silently falling back to
    // using that raw, unexpanded phrase as the literal final scene (skipping
    // every subject/location/action rule in AESTHETIC_SYSTEM_PROMPT entirely).
    await supabase
      .from('uploads')
