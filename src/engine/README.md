...
Essentia features
  ...
  → Gemini Prompt Compiler     (compiler/geminiCompiler.js)  ← optional, story only
...
| **Compiler** | `compiler/` | STORYTELLER. Gemini, demoted: emits only structured scene JSON (subject/pose/expression/action/narrative/symbolism), constrained by the DNA. Forbidden from naming any technical decision. |
| **Assembler** | `assembler/` | COMPILER. Deterministic weld of scene + DNA + technique suffix + guardrails into one prompt. Almost no creative logic (only medium-coherence: an illustrated medium suppresses the camera chain). |
